...
channels_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "config", "channels.json"
)
ambient_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "config", "ambient.json"
)
with open(channels_path, "r") as f:
    channels = json.load(f)
    channels = [c for c in channels.keys() if "pt" in c]
    with client.control.acquire(
        name="ambientization", read=channels, write=None
    ) as auto:
        time.sleep(1)
