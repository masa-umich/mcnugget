...
auto = client.control.acquire(
    name="Demo Autosequence", read=READ_FROM, write=WRITE_TO, write_authorities=100
)
...
def pressurize(auto: synnax.control.Controller, target_pressure):
    return auto[TANK_PRESSURE] > target_pressure


def depressurize(auto: synnax.control.Controller, target_pressure):
    return auto[TANK_PRESSURE] <= target_pressure
...
try:
    ...
    for step in target_steps:
        print(f"pressurizing to {step} psi")
        press_valve.open()
        auto.wait_until(lambda c: pressurize(auto, step))
        press_valve.close()
        print(f"pressurization to {step} psi complete")
        time.sleep(PAUSETIME)

    while True:
        user = input("Enter how much you want to depressurize the system in psi: ")
        try:
            depressurize_step = float(user)
            if 0 <= depressurize_step < 500:
                break
            else:
                print("Please enter a value between 0 and 500.")
        except ValueError:
            print("Invalid input. Please enter a numeric value.")

    print(f"Depressurizing to {depressurize_step} psi")
    press_vent.open()
    auto.wait_until(lambda c: depressurize(auto, depressurize_step))
    press_vent.close()
    print(f"Depressurization complete")
