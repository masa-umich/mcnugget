...
rc.e = 0.05e-3; % m
% fuel channel height
rc.h = convlength(0.035, 'in', 'm');
% fuel channel width
rc.w = convlength(5/32, 'in', 'm');
% number of fuel channels
rc.n = 96;
% percent soot resistance
soot_percent = 1;
% chamber wall thickness
c.t = convlength(0.06, 'in', 'm');
...
% chamber length
% c.L = 0.21902; % m
c.L = convlength(11.207, 'in', 'm');
% modulus
% c.E = 110e9; % Pa copper
c.E = 71e9; % Pa Al 7075
% coefficient of thermal expansion
% c.a = 10.2 * 1.8e-6; % copper
c.a = 12.8e-6 * 1.8; % 1/K Al-7075
% poisson ratio
% c.v = 0.181; % copper
c.v = 0.33; % Al-7075
...
% c.k = 345; % W/m-K copper
c.k = 154; % W/m-K Al-7075
...
v(1:ni) = mdot ./ (rc.A*rc.n * rho(1:ni));
...
fprintf('Gas Convective Coefficient at nozzle inlet: %.0f W/m^2-K\n', hg(np))
fprintf('Liquid Convective Coefficient at nozzle inlet: %.0f W/m^2-K\n', hl(np))
fprintf('Heat Flux at nozzle inlet: %.1f MW/m^2\n', q(np)/1e6)
fprintf('Maximum Coolant Temperature: %.0f K\n', max(Tc))

%%

% convection increase factor to account for no radiation in ansys
rad_adj = 1 + q_rad./q;
rad_adj(isinf(rad_adj)) = 1;
writematrix([x; ones(size(init))*Taw; hge; Tc; hl]', 'convection.xlsx')
fprintf('Results written to convection.xlsx\n')
