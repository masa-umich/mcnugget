function [c_t_list,dp_list,sigma_list,Twh_list] = ...
    marpa_study(rc_w_min, rc_w_max, rc_w_num)

rc_w_list = linspace(rc_w_min, rc_w_max, rc_w_num);
init = zeros(size(rc_w_list));
c_t_list = init;
dp_list = init;
sigma_list = init;
Twh_list = init;
...
c_t_list(i) = c.t;
dp_list = convpres(p(end)-p(1), 'Pa', 'psi');
sigma_list = convpres(min(sigma_di),'Pa','psi');
Twh_list = max(Twh);
