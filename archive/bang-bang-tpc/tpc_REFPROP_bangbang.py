...
class Gas:
    ...
    def __init__(self, P_0: float, T_0: float, V_0: float, gas: str = "Nitrogen"):
        ...
        self.internal_energy = self.RP.REFPROPdll(
            gas, "PT", "U", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.density = self.RP.REFPROPdll(
            gas, "PT", "D", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.m = self.density * V_0  # Mass of gas, conserved [kg]
        self.gamma = self.RP.REFPROPdll(
            gas, "PT", "G", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
    ...
    def update_state(self, P: float, S: float):
        self.internal_energy = self.RP.REFPROPdll(
            self.gas, "PS", "U", self.MASS_BASE_SI, 0, 0, P, S, [1.0]
        ).Output[0]
        self.density = self.RP.REFPROPdll(
            self.gas, "PS", "D", self.MASS_BASE_SI, 0, 0, P, S, [1.0]
        ).Output[0]
        self.temperature = self.RP.REFPROPdll(
            self.gas, "PS", "T", self.MASS_BASE_SI, 0, 0, self.P, self.density, [1.0]
        ).Output[0]

    def update_tank(self, P: float, E: float):
        self.internal_energy = self.RP.REFPROPdll(
            self.gas, "PE", "U", self.MASS_BASE_SI, 0, 0, P, S, [1.0]
        ).Output[0]
        self.density = self.RP.REFPROPdll(
            self.gas, "PE", "D", self.MASS_BASE_SI, 0, 0, P, S, [1.0]
        ).Output[0]
        self.m = self.rho * V_0  # Mass of gas, conserved [kg]
        self.gamma = self.RP.REFPROPdll(
            self.gas, "PE", "G", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.enthalpy = self.RP.REFPROPdll(
            self.gas, "PE", "H", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.entropy = self.RP.REFPROPdll(
            self.gas, "PE", "S", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.compressibility = self.RP.REFPROPdll(
            self.gas, "PE", "Z", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.specific_heat = self.RP.REFPROPdll(
            self.gas, "PE", "Cp", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.temperature = self.T.REFPROPdll(
            self.gas, "PE", "T", self.MASS_BASE_SI, 0, 0, self.P, self.density, [1.0]
        ).Output[0]
...
Area_Init_Fuel = (
    (0 * V0_Ullage_F + vdot_Fuel * P_Fuel_Tank) / (C_mdot * R_N2 * 290) * 1550
)
Area_Init_LOx = (
    Cf * (0 * V0_Ullage_L + vdot_LOx * P_LOx_Tank) / (C_mdot * R_N2 * 160) * 1550
)
A_max_F = (dpdt * V0_Ullage_F + vdot_Fuel * P_Fuel_Tank) / (C_mdot * R_N2 * 290) * 1550
A_max_L = (
    Cf * (dpdt * V0_Ullage_L + vdot_LOx * P_LOx_Tank) / (C_mdot * R_N2 * 160) * 1550
)
...
for y in range(N):
    ...
    N2_C = N2_C.update_state(COPV_pressures[(N - 1) - y], entropy)
    ...
    for z in range(n):
        ...
        def state_func(x):
            return [
                ...
                x[9]
                - E_Ull_L_prev
                - x[7] * N2_C.enthalpy * dt
                + P_LOx_Tank * vdot_LOx * dt,
            ]
    ...
    delta_m_fuelgas = delta_m * (
        Fuel_gas_mdots[(N - 1) - y]
        / (Fuel_gas_mdots[(N - 1) - y] + LOx_gas_mdots[(N - 1) - y])
    )
    delta_m_LOxgas = delta_m * (
        LOx_gas_mdots[(N - 1) - y]
        / (Fuel_gas_mdots[(N - 1) - y] + LOx_gas_mdots[(N - 1) - y])
    )
    delta_E_fuelgas = delta_E * (
        Fuel_gas_mdots[(N - 1) - y]
        / (Fuel_gas_mdots[(N - 1) - y] + LOx_gas_mdots[(N - 1) - y])
    )
    delta_E_LOxgas = delta_E * (
        LOx_gas_mdots[(N - 1) - y]
        / (Fuel_gas_mdots[(N - 1) - y] + LOx_gas_mdots[(N - 1) - y])
    )
    ...
    E_Ullage_Fuel = (
        E_Ullage_Fuel + delta_E_fuelgas - P_Fuel_Tank * (V_Ullage_Fuel - V_Ull_F_prev)
    )
    E_Ullage_LOx = (
        E_Ullage_LOx
        + delta_E_LOxgas
        - ((Cf * delta_E_LOxgas) - delta_E_LOxgas)
        - P_LOx_Tank * (V_Ullage_LOx - V_Ull_L_prev)
    )

    # New tank states
    e_Ullage_Fuel = E_Ullage_Fuel / m_Ullage_Fuel
    e_Ullage_LOx = E_Ullage_LOx / m_Ullage_LOx
    N2_F = Gas.update_tank(P_Fuel_Tank, e_Ullage_Fuel)
    N2_LOx = Gas.update_tank(P_LOx_Tank, e_Ullage_LOx)
    ...
    C_mdot = (
        ...
        * (
            ...
            * N2_C.density
            * N2_C.pressure
            * (2 / (N2_C.gamma + 1)) ** ((N2_C.gamma + 1) / (N2_C.gamma - 1))
        )
        ** 0.5
    )
    ...
    Isoe_plus10_F[(N - 1) - y] = (
        (
            dpdt * (V_Ullage_Fuel) ** 2
            + (delta_m_fuelgas / N2_F.density / delta_t) * N2_F.pressure * V0_Ullage_F
        )
        / (C_mdot * R_Fuel * (N2_F.temperature + 273.15) * V0_Ullage_F * Z_Fuel)
        * 1550
    )
    Isoe_plus10_L[(N - 1) - y] = (
        Cf
        * (
            dpdt * (V_Ullage_LOx) ** 2
            + (delta_m_LOxgas / N2_LOx.density / delta_t)
            * N2_LOx.pressure
            * V0_Ullage_L
        )
        / (C_mdot * R_LOx * (N2_LOx.temperature + 273.15) * V0_Ullage_L * Z_LOx)
        * 1550
    )
...
quarter_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [0.25 * CdA_Valve, 0.25 * CdA_Valve], "-g", label="1/4 Tescom"
)
half_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [0.5 * CdA_Valve, 0.5 * CdA_Valve], "-g", label="1/2 Tescom"
)
one_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [CdA_Valve, CdA_Valve], "-g", label="1 Full Tescom"
)
plt.ylim([0, 0.03])
...
one_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [CdA_Valve, CdA_Valve], "-g", label="1 Tescom"
)
two_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [2 * CdA_Valve, 2 * CdA_Valve], "-g", label="2 Tescoms"
)
three_valve = plt.plot(
    [P_C[0], P_C[N - 1]], [3 * CdA_Valve, 3 * CdA_Valve], "-g", label="3 Tescoms"
)
three_point_five_valve = plt.plot(
    [P_C[0], P_C[N - 1]],
    [3.5 * CdA_Valve, 3.5 * CdA_Valve],
    "-g",
    label="3 1/2 Tescoms",
)
plt.ylim([0, 0.1])
