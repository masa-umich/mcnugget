...
N2_C = Fluid(FluidsList.Nitrogen).with_state(
    Input.pressure(4000 * 6894.76), Input.temperature(16.85)
)
entropy = N2_C.entropy
COPV_pressures = np.linspace(1200 * 6894.76, 4000 * 6894.76, N)

# Initial Fuel Ullage Gas Properties
N2_F = Fluid(FluidsList.Nitrogen).with_state(
    Input.pressure(P_Fuel_Tank), Input.temperature(16.85)
)
...
N2_LOx = Fluid(FluidsList.Nitrogen).with_state(
    Input.pressure(P_LOx_Tank), Input.temperature(160 - 273.15)
)
...
for y in range(N):
    ...
    N2_C = Fluid(FluidsList.Nitrogen).with_state(
        Input.pressure(COPV_pressures[(N - 1) - y]), Input.entropy(entropy)
    )
    ...
    e_Ullage_LOx = E_Ullage_LOx / m_Ullage_LOx
    N2_F = Fluid(FluidsList.Nitrogen).with_state(
        Input.pressure(P_Fuel_Tank), Input.internal_energy(e_Ullage_Fuel)
    )
    N2_LOx = Fluid(FluidsList.Nitrogen).with_state(
        Input.pressure(P_LOx_Tank), Input.internal_energy(e_Ullage_LOx)
    )
