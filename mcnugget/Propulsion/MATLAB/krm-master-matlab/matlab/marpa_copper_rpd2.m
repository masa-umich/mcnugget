...
% liner transient geometry
% liner cross sectional area
liner.Acs = pi/4*(rc.di^2-c.di^2); % 
% liner mass per length
liner.m_l = liner.Acs*c.rho; % kg/m
% liner heat capacity per length
liner.cp_l = liner.m_l*c.cp; % J/kg-K-m
% inside surface area per length (perimeter)
liner.A_l = c.di*pi;
...
SFY = sty ./ abs(sigma_di);
% safety factor to ultimate
SFU = stu ./ abs(sigma_di);

fprintf('\n')
fprintf('Liner Wall Thickness: %.4f in\n', convlength(c.t, 'm', 'in'))
...
fprintf('Minimum Liner Yield Strength: %.1f ksi\n', convpres(min(sty),'Pa','psi')/1000)
...
% element of the chamber wall
% surface area per axial length
ce.sa = c.di*pi;
% mass per axial length
ce.m = pi/4*((c.di+c.t)^2-c.di^2)*c.rho;
% dh/dT
ce.dh_dT = (rpa.hg(600)-rpa.hg(300))/(600-300);
% hg(T)
ce.hg = @(T) rpa.hg(300) + ce.dh_dT*(T-300);
% effective htc
ce.hge = @(T) 1./(1/ce.hg(T) + soot_percent/c.hs);
% liquid convective coefficient
ce.hl = hl(np);
% liquid temperature
ce.Tc = Tc(np);
% heat flux in
q_in  = @(T) ce.hge(T)*(Taw-T);
% heat flux out
q_out = @(T) ce.hl*(T-ce.Tc);
% net heat flux in
q_net = @(T) q_in(T)-q_out(T);

f = @(T) q_net(T) * ce.sa / (ce.m * c.cp);

[t,T] = ode45(@(t,y) f(y), [0 12], ce.T0);
ce.t = t;
ce.T = T;
...
title('Liner Transient Heating')
saveas(gcf, 'Liner Transient Heating.jpg')

if 0
% liner transient station
npl = np;
% initial temperature
liner.T = 300; % K
% time
...
liner.T_prev = liner.T - eps_T*2;

% regen convective coefficient
liner.hl = hl(npl); % W/m^2-K

% effective hot gas convective coefficient
liner.hge = hge(npl); % W/m^2-K

% combustion gas temperature = Taw

% fuel temperature
liner.Tc = Tc(npl);

while abs(liner.T - liner.T_prev) > eps_T
    q_in = liner.hge * (Taw-liner.T);
    q_out = liner.hl * (liner.T-liner.Tc);
    q_net = q_in - q_out;
    liner.T_prev = liner.T;
    liner.T = liner.T + q_net*liner.A_l/(liner.m_l*c.cp)*dt;
%     fprintf('T(%.3f) = %.0f\n', t, liner.T)
...
% hge = 1./(1./hg + soot_percent/c.hs);
% 
% output = flipud([x; Twh; Twc; Tc; q; hge; hl]');
% 
% outputfile = 'marpa_results.xlsx';
% 
% writematrix(output, outputfile)
