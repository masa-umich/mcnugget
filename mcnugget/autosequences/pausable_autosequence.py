...
# A PNID is a Piping and Instrumentation Diagram. They are usually quite complicated - below is a code representation of one.
"""
                                   ____
                                  |    |
           ---<<<---[A]---<<<-----| P1 |---<<<---[C]---<<<---
...
def run_auto():
    ...
    with client.control.acquire(name="Demo Autosequence",
        ...
        syauto.pressurize(auto=auto, valve_s=fuel_press, pressure_s=FUEL_PT, target=TARGET_1, inc=(TARGET_1 / 10), max=1000)
...
def outer_shell(running: threading.Thread):
    running.start()
    while True:
        if pause_triggered:
            running.stop()

main = threading.Thread(target=run_auto)
outer_shell(main)