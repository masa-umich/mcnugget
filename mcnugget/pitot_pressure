...
A = 1 # CHANGE THIS
press_ratio = stagnation_pressure / static_pressure # Calculate stagnation to static pressure ratio

# Calculate Mach number
M = (((press_ratio)**((gamma-1)/gamma)-1)*(2/(gamma-1)))**.5

calc_stag_T = static_temp*(1+((gamma-1)/2)*M**2)

# Calculate Mass Flow Rate
letters = (1+((gamma-1)/2)*(M**2))**((gamma+1)/(2-2*gamma))
m_dot = (stagnation_pressure/(np.sqrt(R*calc_stag_T)))*A*np.sqrt(gamma)*M*letters
