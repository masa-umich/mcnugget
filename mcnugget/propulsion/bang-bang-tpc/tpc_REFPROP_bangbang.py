...
class Gas:
    ...
    def __init__(self, P_0: float, T_0: float, V_0: float, gas: str = "Nitrogen"):
        ...
        self.P = P_0    # [Pa]
        self.T = T_0    # [K]
        self.internal_energy = self.RP.REFPROPdll(gas,"PT","U",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.density = self.RP.REFPROPdll(gas,"PT","D",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.m = self.density*V_0  # Mass of gas, conserved [kg]
        self.gamma = self.RP.REFPROPdll(gas,"PT","G",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.enthalpy = self.RP.REFPROPdll(gas,"PT","H",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.entropy = self.RP.REFPROPdll(gas,"PT","S",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.compressibility = self.RP.REFPROPdll(gas,"PT","Z",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.specific_heat = self.RP.REFPROPdll(gas,"PT","Cp",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        
   
    def update_state(self, P: float, S: float):
        self.internal_energy = self.RP.REFPROPdll(self.gas,"PS","U",self.MASS_BASE_SI,0,0,P,S,[1.0]).Output[0]
        self.density = self.RP.REFPROPdll(self.gas,"PS","D",self.MASS_BASE_SI,0,0,P,S,[1.0]).Output[0]
        self.temperature = self.RP.REFPROPdll(self.gas, "PS", "T", self.MASS_BASE_SI, 0, 0, self.P, self.density, [1.0]).Output[0]
    
    def update_tank(self, P: float, E: float):
        self.internal_energy = self.RP.REFPROPdll(self.gas,"PE","U",self.MASS_BASE_SI,0,0,P,S,[1.0]).Output[0]
        self.density = self.RP.REFPROPdll(self.gas,"PE","D",self.MASS_BASE_SI,0,0,P,S,[1.0]).Output[0]
        self.m = self.rho*V_0  # Mass of gas, conserved [kg]
        self.gamma = self.RP.REFPROPdll(self.gas,"PE","G",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.enthalpy = self.RP.REFPROPdll(self.gas,"PE","H",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.entropy = self.RP.REFPROPdll(self.gas,"PE","S",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.compressibility = self.RP.REFPROPdll(self.gas,"PE","Z",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.specific_heat = self.RP.REFPROPdll(self.gas,"PE","Cp",self.MASS_BASE_SI,0,0,self.P,self.T,[1.0]).Output[0]
        self.temperature = self.T.REFPROPdll(self.gas, "PE", "T", self.MASS_BASE_SI, 0, 0, self.P, self.density, [1.0]).Output[0]
...
R_N2 = 296.80
C_mdot = Cd * COPV_max * (gamma/(R_N2*290))**0.5 * (2/(gamma+1))**((gamma+1)/(2*(gamma-1)))
...
for y in range(N):
    ...
    C_mdot = (Cd*(N2_C.gamma*N2_C.density*N2_C.pressure*(2/(N2_C.gamma+1))**((N2_C.gamma+1)/(N2_C.gamma-1)))**0.5)
