...
class EngineModel:
    def __init__(
        ...
        C_F: float = 1.351,
        C_star: float = 1519.4,
        C_star_eff: float = 0.85,
        P_c_ideal: float = 309.5,
    ):
...
class GasState:
    ...
    def __init__(self, P_0: float, T_0: float, V_0: float, gas: str = "Nitrogen"):
        ...
        self.E = self.RP.REFPROPdll(
            gas, "PT", "E", self.MASS_BASE_SI, 0, 0, self.P, self.T, [1.0]
        ).Output[0]
        self.rho = self.RP.REFPROPdll(
    ...
    def update_state(self, dV: float, new_V_ullage: float):
        ...
        self.P = self.RP.REFPROPdll(
            self.gas, "ED", "P", self.MASS_BASE_SI, 0, 0, self.E, self.rho, [1.0]
        ).Output[0]
        self.T = self.RP.REFPROPdll(
            self.gas, "ED", "T", self.MASS_BASE_SI, 0, 0, self.E, self.rho, [1.0]
        ).Output[0]

    def get_state_string(self) -> str:
        return "[GAS STATE] P: {0:.2f} psi | T: {1:.2f} K | ".format(
            self.P / PSI_TO_PA, self.T
        ) + "Energy: {0:.3f} kJ/kg | Density: {1:.3f} kg/m^3".format(
            self.E / 1e3, self.rho
        )
...
class PropTank:
    def __init__(
        ...
        self.gas_state = GasState(
            P_ullage_0 * PSI_TO_PA, T_ullage_0, self.V_ullage, "Nitrogen"
        )
        self.CdA = CdA
    ...
    def get_state_string(self):
        return (
            "[TANK] Propellant mass: {0:.2f} kg | Ullage fraction: {1:.2f}% \n\t".format(
                self.V_prop * self.rho_prop, self.ullage_frac * 100
            )
            + self.gas_state.get_state_string()
        )
...
def solve(engine: EngineModel, ox_tank: PropTank, fuel_tank: PropTank):
    ...
    for i in range(t_range.size):
        ...
        mdot_o, mdot_f, c_star, F_t, P_c = fsolve(
            system_equations, init_guess, args=constants
        )
